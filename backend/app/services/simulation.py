...
Fuente: Guias_30_dias_N1693_DATOS_PRACTICA.xlsx, seis hojas GO, filas 13:42.
El usuario declara PRETEST real y la fuente identifica ambas fases como sintéticas:
...
from ..models import Asset, AuditLog, ResearchSample, SimulationStudy, SimulationAsset

ALGORITHM = 'daily-reconstruction-v1'
    ...
    try:
            ...
            for day, row in enumerate(values[12:42], 1):
                ...
                for phase, offset in [('PRETEST',0),('POSTTEST',6 if metric == 'TPGR' else 5)]:
                    ...
                    pair[phase] = {'date': stamp.date().isoformat(), 'denominator': denominator, 'numerator': numerator,
                                   ...
                                   'nature': 'USER_DECLARED_REAL_AGGREGATE' if phase == 'PRETEST' else 'SIMULATED_SOURCE_AGGREGATE'}
        ...
        if any('SINTÉTICOS' in str(g['sourceNotice']).upper() for g in guides.values()):
            discrepancies.append({'code':'SOURCE_DECLARATION_CONFLICT', 'severity':'REQUIERE_ACLARACION_DOCUMENTAL',
                                  'message':'El usuario declara pretest real; la fila 10 del libro identifica ambas fases como sintéticas. No se ha verificado evidencia primaria.'})
...
def import_guides(path, user_id):
    ...
    existing = db.session.get(SimulationStudy,study_id)
    if existing: return existing, False
    generated = reconstruct(guides,census); checks = reconcile(guides,generated)
    if not all(check['passed'] for check in checks): raise ValueError('La reconstrucción no concilia con el archivo.')
    ...
    try:
        study = SimulationStudy(id=study_id,filename=Path(path).name,source_hash=source_hash,source_blob=blob,
                                source_cells=archive,guides=guides,manifest=manifest,created_by=user_id)
        db.session.add(study); db.session.flush()
        for values in generated: db.session.add(SimulationAsset(study_id=study_id,**values))
