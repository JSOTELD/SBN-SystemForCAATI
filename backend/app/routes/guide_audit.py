...
from ..models import GuideStudy, GuideAsset, AuditLog
...
def study_or_404(study_id):
    study = db.session.get(GuideStudy, study_id)
...
def details(study_id):
    return [{'sample_code':a.sample_code,'snapshot':a.snapshot,'pre':a.pre,'post':a.post}
            for a in db.session.scalars(db.select(GuideAsset).where(GuideAsset.study_id==study_id).order_by(GuideAsset.sample_code)).all()]
...
def assets(study_id):
    ...
    query=db.select(GuideAsset).where(GuideAsset.study_id==study_id)
    day=request.args.get('day',type=int)
    # El total es pequeño (1693 pares); filtro por JSON portable entre SQLite/MySQL.
    rows=db.session.scalars(query.order_by(GuideAsset.sample_code)).all()
