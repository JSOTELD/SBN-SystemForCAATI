"""Validación aislada de guías, reconstrucción, permisos y expediente."""
import copy
import hashlib
import io
import json
...
from app.models import User, Asset, ResearchSample, Observation, SimulationAsset
from app.services.guide_audit import import_guides, reconstruct, reconcile
    ...
    def test_import_audit_and_isolation(self):
        ...
        with app.app_context():
            ...
            try:
                ...
                self.assertEqual(reconstruct(study.guides,[{'sample_code':r['sample_code'],'snapshot':r['snapshot']} for r in generated]),generated)
                ...
                row=db.session.scalar(db.select(SimulationAsset));row.pre={**row.pre,'durationMs':row.pre['durationMs']+1};db.session.commit()
